...
class Road(object):
  ...
  def cull(self):
    ...
    center_s = ego.s
    claimed = set([(v.lane, v.s) for v in list(self.vehicles.values())])
    for v_id, v in list(self.vehicles.items()):
      if v.s > (center_s + self.update_width / 2) or v.s < (center_s - self.update_width / 2):
        ...
        placed = False
        while not placed:
          lane_num = random.choice(list(range(self.num_lanes)))
          ds = random.choice(list(range(self.update_width/2-15,self.update_width/2 -1)))
