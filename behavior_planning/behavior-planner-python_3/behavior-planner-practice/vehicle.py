class Vehicle(object):
  ...
  def _max_accel_for_lane(self, predictions, lane, s):
    ...
    in_front = [v for (v_id, v) in list(predictions.items()) if v[0]['lane'] == lane and v[0]['s'] > s ]
    if len(in_front) > 0:
  ...
  def realize_prep_lane_change(self, predictions, direction):
    ...
    ids_and_vehicles = [(v_id, v) for (v_id, v) in list(predictions.items()) if v[0]['lane'] == lane and v[0]['s'] <= self.s]
